...
{//This function sets up the two shapes we need for this example.
	...
	box1.faceNormalSet.push_back(glm::cross((p5 - p2), (p3 - p5)));
	box1.faceNormalSet.push_back(glm::cross((p5 - p3), (p3 - p4)));
	...
	//Setting up box 2
	box2.origin = glm::vec3(0.5f, 0.2f, 0.0f);
	...
	box1.linearVelocity = glm::vec3(0.5f, 0.0f, 0.0f);
	...
	box2.inverseMass = 1.0f / box2.mass;
	...
	box2.inertiatensor = glm::mat3(1.0f);
	box2.inertiatensor[0][0] = box2.mass * ((0.4f)*(0.4f)) / 6.0f;
	box2.inertiatensor[1][1] = box2.inertiatensor[0][0];
	box2.inertiatensor[2][2] = box2.inertiatensor[0][0];
}
...
{	
	...
	{	
		...
		glm::mat3 inertiaA = box1.inertiatensor;
		inertiaA = glm::mat3(box1.RotationMat) * inertiaA * glm::transpose(glm::mat3(box1.RotationMat));

		glm::mat3 inertiaB = box2.inertiatensor;
		inertiaB = glm::mat3(box2.RotationMat) * inertiaB * glm::transpose(glm::mat3(box2.RotationMat));
		...
		float denomComponent = glm::dot((glm::cross(glm::inverse(inertiaA) * glm::cross(Ra, n), Ra) + glm::cross(glm::inverse(inertiaB) *glm::cross(Rb, n), Rb)), n);

		// Caculate the impulse
		float j = (-(1 + e) * glm::dot(Vab, n)) / (box1.inverseMass + box2.inverseMass + denomComponent);
		...
		glm::vec3 Wa2 = glm::inverse(inertiaA) * L1;
		glm::vec3 Wb2 = glm::inverse(inertiaB) * L2;
...
void update(float t)
{
	...
	box1.origin = EulerIntegrator(box1.origin, t, box1.linearVelocity);
	box2.origin = EulerIntegrator(box2.origin, t, box2.linearVelocity);
	...
	{
		boxOutfocus->origin += n;									// if collision occours then move the other object by MTV		(Decoupling)
...
void main()
{
	...
	window = glfwCreateWindow(800, 800, "3D convex hull Collision resolution", nullptr, nullptr);

	std::cout << "\n This program demonstrates resolution of collision between two Conmvex hulls\n\n\n\n\n\n\n\n\n\n";
